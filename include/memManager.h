...
class MonotonicBufferResource : public std::pmr::memory_resource {
 public:
  ...
  MonotonicBufferResource(size_t bufferSize)
      : bufferSize(bufferSize), sizeLeft(bufferSize), buffer(bufferSize) {
 ...
 protected:
  void* do_allocate(size_t bytes, size_t alignment) override {
    if (sizeLeft < bytes) throw std::bad_alloc();
    if (std::align(alignment, bytes, bufferPosition, sizeLeft)) {
      ...
      if ((iptr % alignment) != 0) throw std::runtime_error("Alignment failed");
      return result;
    }
    throw std::bad_alloc();
  }
  void do_deallocate(void* /*p*/, size_t /*bytes*/,
    ...
    throw std::runtime_error(
        " Function not supported in monotonic increasing buffer");
  }
...
class SynchronizedMonotonicBufferRessource : public std::pmr::memory_resource {
 ...
 public:
  SynchronizedMonotonicBufferRessource(size_t bufferSize)
      : parent(bufferSize){};
 ...
 protected:
  void* do_allocate(size_t bytes, size_t alignment) override {
    ...
    std::lock_guard<std::mutex> allocLock(allocteMut);
