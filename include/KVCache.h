...
struct FilterPoolNode {
  ...
  FilterPoolNode() { filter = new Bloomfilter(); }
};
...
class FilterPool {
 public:
  FilterPool(MonotonicBufferRing<SkipListNode> *buffer) {
    create(buffer);
    filterCreator =
        std::thread(std::bind(&FilterPool::filter_thread, this, buffer));
    bufferClearer =
        std::thread(std::bind(&FilterPool::buffer_thread, this, buffer));
  ...
  restart:
    ...
    int back = start;
    if (RING_SUB(start, pre_oldest, kMaxFilters) <
        RING_SUB(oldest, pre_oldest, kMaxFilters)) {
      return -1;
    }
    if (start < oldest) {
      back += kMaxFilters;
 ...
 private:
  ...
  skRestart:
    ...
    for (int i = 0; i < n; i++) {
      ...
      auto sk_cur_v =
          cutil::read_lock_or_restart(fpn->skiplist_lock_, sk_need_restart);
      if (sk_need_restart) {
        goto skRestart;
      }
      cutil::upgrade_to_write_lock_or_restart(fpn->skiplist_lock_, sk_cur_v,
                                              sk_need_restart);
      if (sk_need_restart) {
        goto skRestart;
      }
  ...
  static void clear_thread(FilterPool *fp) {
    ...
    while (true) {
      ...
      int i = 0;
      while (fp->filters[RING_ADD(fp->oldest, i, kMaxFilters)].endTS <
             fp->oldest_time) {
        i++;
      }
      if (i > 0) {
  ...
  static void filter_thread(FilterPool *fp,
    ...
    // MonotonicBufferRing<SkipListNode> * buffer =
    // static_cast<MonotonicBufferRing<SkipListNode> *>(args[1]);
    struct timespec sleep_time;
    sleep_time.tv_nsec = filter_thread_interval;
    sleep_time.tv_sec = 0;
    while (true) {
      ...
      size_t cur_offset;
      size_t cur_distance = buffer->check_distance(
          fp->filters[fp->latest].start_offset, cur_offset);
  ...
  bool isEmpty() { return oldest == latest; }

  bool isFull() { return RING_ADD(latest, 1, kMaxFilters) == oldest; }
  ...
  restart:
    ...
    for (int i = kMaxLevel - 1; i >= 0; i--) {
      ...
      if (i > 0) {
        ...
        auto f_v = cutil::read_lock_or_restart(
            Latest_lock_buffer[filter->my_fliper][cur_pos - start_pos][i - 1],
            need_restart);
        if (need_restart) {
