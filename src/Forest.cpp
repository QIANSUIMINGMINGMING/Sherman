...
thread_local GlobalAddress
    BForest::stack_page_addr_buffer[define::kMaxLevelOfTree];

BInsertedEntry new_inserted_entries[define::kMaxNumofInternalInsert *
                                    kMaxBatchInsertCoreNum];
int new_inserted_entries_cnts[kMaxBatchInsertCoreNum];

BForest::BForest(DSM *dsm, int CNs, uint16_t tree_id)
    : dsm(dsm), tree_num(CNs), tree_id(tree_id) {
  ...
  dsm->write_sync((char *)next_level_page, next_level_page_addr,
                  kInternalPageSize);
...
GlobalAddress BForest::get_root_ptr_ptr(uint16_t id) {
  GlobalAddress addr;
  addr.nodeID = 0;
  addr.offset = define::kRootPointerStoreOffest + sizeof(GlobalAddress) * id;

  return addr;
}

extern RootCache *rootCaches;
GlobalAddress BForest::get_root_ptr(CoroContext *cxt, int coro_id,
                                    uint16_t id) {
...
void BForest::batch_insert(KVTS *kvs, int full_number, int thread_num) {
  ...
  for (int i = 0; i < thread_num; i++) {
    batch_insert_threads[i] = std::thread(std::bind(
        &BForest::batch_insert_leaf, this, kvs, i * (per_thread_num + 2) - 1,
        per_thread_num + 2, full_number, i, nullptr, 0));
  }
  for (int i = 0; i < thread_num; i++) {
...
void BForest::batch_insert_internal(int thread_num) {
  ...
  for (uint64_t i = 0; i < cur_level; i++) {
    new_inserted_entries[i].visit_all([&](auto &x) {
      std::sort(x.second.begin(), x.second.end(),
                [](const BInternalEntry &a, const BInternalEntry &b) {
                  return a.key < b.key;
                });
      bool need_split = x.second.size() > kBInternalCardinality;
      uint64_t split_num =
          ((x.second.size() - 1) / (kBInternalCardinality / 2)) + 1;

      if (!need_split) {
        auto page_buffer = (dsm->get_rbuf(0)).get_page_buffer();
        auto page_header =
            (BHeader *)(page_buffer + (STRUCT_OFFSET(BInternalPage, hdr)));
        auto page = (BInternalPage *)page_buffer;
        // set page
        assert(new_inserted_headers.find(x.first) !=
               new_inserted_headers.end());
        ...
        while (remain_num > 0) {
          ...
          for (int cur_page_i = 0; cur_page_i < page_num; cur_page_i++) {
            ...
            cur_kv_pos++;
            for (size_t cur_page_pos = 0; cur_page_pos < per_page_num - 1;
                 cur_page_pos++) {
              cur_new_page->records[cur_page_pos].key =
                  x.second[cur_kv_pos].key;
              cur_new_page->records[cur_page_pos].ptr =
                  x.second[cur_kv_pos].ptr;
              assert(cur_new_page->records[cur_page_pos].ptr !=
                     GlobalAddress::Null());
              cur_kv_pos++;
              last_index++;
            ...
            cur_new_page->hdr.highest =
                (cur_page_i == page_num - 1 && remain_num - page_num == 0)
                    ? split_highest
                    : x.second[cur_kv_pos].key;
            ...
            new_inserted_entries[i + 1].visit(
                cur_parent_addr_ptr.val,
                [&](auto &y) { y.second.push_back(be); });
          }
  ...
  while (need_new_root) {
    new_inserted_entries[cur_level].visit(
        next_level_page_addr.val, [&](auto &x) {
          std::sort(x.second.begin(), x.second.end(),
                    [](const BInternalEntry &a, const BInternalEntry &b) {
                      return a.key < b.key;
                    });
          ...
          if (!need_split) {
            ...
            dsm->write_sync((char *)next_level_page, write_pos,
                            kInternalPageSize, nullptr);
            update_root(cur_level + 1);
          } else {
            // assign next next level page
            auto next_level_parent_page_buffer =
                dsm->get_rbuf(0).get_next_level_buffer();
            auto next_level_parent_page_addr = dsm->alloc(kInternalPageSize);
            BInternalPage *next_level_parent_page =
                new (next_level_parent_page_buffer) BInternalPage;
            ...
            uint64_t next_level_parent_page_addr_val =
                next_level_parent_page_addr.val;
            new_inserted_entries->emplace(next_level_parent_page_addr_val,
                                          std::vector<BInternalEntry>());
            ...
            while (remain_num > 0) {
              ...
              auto pages = (dsm->get_rbuf(0)).get_split_page_buffer(page_num);
              for (int cur_page_i = 0; cur_page_i < page_num; cur_page_i++) {
                BInternalPage *cur_new_page =
                    &((BInternalPage *)pages)[cur_page_i];
                int last_index = 0;
                cur_new_page->hdr.lowest = x.second[cur_kv_pos].key;
                cur_new_page->hdr.leftmost_ptr = x.second[cur_kv_pos].ptr;
                cur_kv_pos++;
                ...
                be.key = cur_new_page->hdr.lowest;
                be.ptr = GADD(store_addr, cur_page_i * kInternalPageSize);
                new_inserted_entries[cur_level + 1].visit(
                    cur_parent_addr_ptr.val,
                    [&](auto &y) { y.second.push_back(be); });
              }
              dsm->write_sync(pages, store_addr, page_num * kLeafPageSize,
                              nullptr);
              remain_num -= page_num;
            }
...
void BForest::batch_insert_leaf(BForest *forest, KVTS *global_kvs, int start,
                                int cnt, int full_number, int thread_id,
  ...
  BatchInsertFlag r_flag =
      kvs[cnt - 1].k >= ((BLeafPage *)(stack_page_buffer[0]))->hdr.highest
          ? BatchInsertFlag::RIGHT_DIFF_PAGE
          : BatchInsertFlag::RIGHT_SAME_PAGE;
  ...
  BatchInsertFlag l_flag =
      (kvs[0].k < ((BLeafPage *)(stack_page_buffer[0]))->hdr.lowest ||
       start == 0)
          ? BatchInsertFlag::LEFT_DIFF_PAGE
          : BatchInsertFlag::LEFT_SAME_PAGE;
  ...
  if (r_flag == BatchInsertFlag::RIGHT_SAME_PAGE) {
    while (pre_same_page_kv < kBLeafCardinality) {
      if (global_kvs[start + cnt - 1 + pro_same_page_kv].k >=
          last_leaf_highest) {
        break;
      }
      pro_same_page_kv++;
  ...
  while (j < cnt - 1) {
    ...
    for (int i = 0; i < cur_level; i++) {
      ...
      if (i == 0) {
        ...
        if (k < i_page->hdr.lowest || k >= i_page->hdr.highest) {
          ...
          if (split_num > 0) {
            forest->split_leaf(kvs, split_start, split_num, split_range,
                               stack_page_addr_buffer[0] == first_leaf_page_addr
                                   ? l_flag
                                   : BatchInsertFlag::LEFT_DIFF_PAGE,
                               BatchInsertFlag::RIGHT_DIFF_PAGE, thread_id, cxt,
                               coro_id);
            split_num = 0;
            split_range = 0;
          } else {
    ...
    BatchInsertFlag set_lflag = leaf_addr == first_leaf_page_addr
                                    ? l_flag
                                    : BatchInsertFlag::LEFT_DIFF_PAGE;
    BatchInsertFlag set_rflag = leaf_addr == last_leaf_page_addr
                                    ? r_flag
                                    : BatchInsertFlag::RIGHT_DIFF_PAGE;
    forest->set_leaf(kvs + j, k, v, need_split, set_lflag, set_rflag,
                     pre_same_page_kv, pro_same_page_kv, cxt, coro_id);
    if (need_split) {
  ...
  if (split_num != 0) {
    BatchInsertFlag llflag = stack_page_addr_buffer[0] == first_leaf_page_addr
                                 ? l_flag
                                 : BatchInsertFlag::LEFT_DIFF_PAGE;
    forest->split_leaf(kvs, split_start, split_num, split_range, llflag, r_flag,
                       thread_id, cxt, 0);
  }
}

void BForest::go_in_leaf(BLeafPage *old_page, int start, Key lowest,
                         Key highest, int &next) {
  for (int i = start; i < kBLeafCardinality; i++) {
    if (old_page->records[i].key >= lowest &&
        old_page->records[i].key < highest &&
        old_page->records[i].value != kValueNull) {
      next = i;
      return;
    }
  }
...
void BForest::calculate_meta(int split_num, BatchInsertFlag l_flag) {
  ...
  int next_level_insert_number =
      l_flag == BatchInsertFlag::LEFT_SAME_PAGE ? split_num : split_num - 1;

  while (next_parent_page_addr != GlobalAddress::Null() &&
         next_level_insert_number > 0) {
    ...
    int old_split_page_num =
        (meta - next_level_insert_number) / kBInternalCardinality;
    next_level_insert_number =
        (meta / kBInternalCardinality) - old_split_page_num;

    if (new_inserted_entries[level - 1].try_emplace(
            next_parent_addr_val, std::vector<BInternalEntry>())) {
  ...
  if (next_level_insert_number > 0) {
    new_inserted_entries[level - 1].try_emplace(next_level_page_addr.val,
                                                std::vector<BInternalEntry>());
  }
}

void BForest::split_leaf(KVTS *kvs, int start, int insert_split_num, int range,
                         BatchInsertFlag l_flag, BatchInsertFlag r_flag,
                         int thread_id, CoroContext *cxt, int coro_id) {
  Key n_lowest = l_flag == BatchInsertFlag::LEFT_SAME_PAGE
                     ? kvs[start].k
                     : ((BLeafPage *)(stack_page_buffer[0]))->hdr.lowest;
  Key n_highest = r_flag == BatchInsertFlag::RIGHT_SAME_PAGE
                      ? kvs[start + range].k
                      : ((BLeafPage *)(stack_page_buffer[0]))->hdr.highest;

  // calculate the number of all entries
  int actual_num = insert_split_num;
  auto old_page = (BLeafPage *)stack_page_buffer[0];
  std::sort(
      old_page->records, old_page->records + kBLeafCardinality,
      [](const BLeafEntry &a, const BLeafEntry &b) { return a.key < b.key; });

  for (int i = 0; i < kBLeafCardinality; i++) {
    if (old_page->records[i].key >= n_lowest &&
        old_page->records[i].key < n_highest &&
        old_page->records[i].value != kValueNull) {
      actual_num++;
    }
  }
  ...
  if (parent_page_addr == GlobalAddress::Null()) {
    ...
    if (new_inserted_entries[0].try_emplace(next_level_page_addr.val,
                                            std::vector<BInternalEntry>())) {
  ...
  while (remain_num != 0) {
    ...
    for (int i = 0; i < page_num; i++) {
      ...
      BHeader *page_header =
          (BHeader *)(page + (STRUCT_OFFSET(BLeafPage, hdr)));
      int last_index = 0;
      for (int j = 0; j < per_page_num; j++) {
        if (cur_pos_kvts < range && cur_pos_old < kBLeafCardinality) {
          if (kvs[cur_pos_kvts].k < old_page->records[cur_pos_old].key) {
            ...
            go_in_leaf(old_page, cur_pos_old + 1, n_lowest, n_highest,
                       cur_pos_old);
          } else {
            page->records[j].key = kvs[cur_pos_kvts].k;
            page->records[j].value = kvs[cur_pos_kvts].v;
      ...
      page_header->lowest =
          i == 0 ? n_lowest : ((BLeafPage *)pages)[i - 1].hdr.highest;
      page_header->highest = (i == page_num - 1 && remain_num - page_num == 0)
                                 ? n_highest
                                 : page->records[last_index - 1].key + 1;
      page_header->last_index = last_index;
      // write to parent
      if (i == 0 && remain_num == split_num &&
          l_flag == BatchInsertFlag::LEFT_DIFF_PAGE && uplayer_key != kKeyMax) {
        ...
        new_inserted_entries[0].visit(parent_page_addr.val,
                                      [&](auto &x) { x.second.push_back(be); });
      }
    }
...
void BForest::set_leaf(KVTS *kvs, const Key &k, const Value &v,
  ...
  for (int i = 0; i < kBLeafCardinality; i++) {
    if (page->records[i].key == k && page->records[i].value != kValueNull) {
      if (page->records[i].value != v) {
        ...
        kvs->v = kValueNull;
        // rdma_write kv
        char *update_addr = (char *)page + (STRUCT_OFFSET(BLeafPage, records) +
                                            i * sizeof(BLeafEntry));
        GlobalAddress remote_addr = GADD(
            addr, STRUCT_OFFSET(BLeafPage, records) + i * sizeof(BLeafEntry));
  ...
  if (l_flag == BatchInsertFlag::LEFT_SAME_PAGE &&
      r_flag == BatchInsertFlag::RIGHT_SAME_PAGE) {
    need_split = true;
    return;
  }
  assert(old != -1);
  if (old >= kBLeafCardinality ||
      (old + pre_kv >= kBLeafCardinality &&
       l_flag == BatchInsertFlag::LEFT_SAME_PAGE) ||
      (old + pro_kv >= kBLeafCardinality &&
       r_flag == BatchInsertFlag::RIGHT_SAME_PAGE)) {
    need_split = true;
    return;
  } else {
    ...
    if (l_flag == BatchInsertFlag::LEFT_SAME_PAGE) {
      for (int i = kBLeafCardinality - 1; i >= 0; i--) {
        if (page->records[i].value == kValueNull) {
          update_addr = (char *)page + (STRUCT_OFFSET(BLeafPage, records) +
                                        i * sizeof(BLeafEntry));
          remote_addr = GADD(
              addr, STRUCT_OFFSET(BLeafPage, records) + i * sizeof(BLeafEntry));
          break;
...
void BForest::set_stack_buffer(int level, const Key &k, CoroContext *cxt,
                               int coro_id) {
  assert(level != 0);
  ...
  for (int i = level; i > 0; i--) {
    ...
    if (i == 1) {
      buffer_header =
          (BHeader *)(page_buffer + (STRUCT_OFFSET(BLeafPage, hdr)));
    } else {
      buffer_header =
          (BHeader *)(page_buffer + (STRUCT_OFFSET(BInternalPage, hdr)));
    }
    ...
    tree_meta.try_emplace_or_visit(
        next_level_ptr_val, (int)(buffer_header->last_index),
        [&](auto &x) { x.second = (int)(buffer_header->last_index); });
  }
}

void BForest::search_stack_buffer(int level, const Key &k,
...
bool BForest::search(const Key &k, Value &v, CoroContext *cxt, int coro_id) {
  assert(dsm->is_register());
...
bool BForest::page_search(GlobalAddress page_addr, const Key &k,
                          BSearchResult &result, CoroContext *cxt,
                          int coro_id) {
  auto page_buffer = (dsm->get_rbuf(coro_id)).get_page_buffer();
  auto header = (BHeader *)(page_buffer + (STRUCT_OFFSET(BLeafPage, hdr)));
...
void BForest::internal_page_search(BInternalPage *page, const Key &k,
                                   BSearchResult &result) {
  assert(k >= page->hdr.lowest);
  assert(k < page->hdr.highest);
...
void BForest::leaf_page_search(BLeafPage *page, const Key &k,
                               BSearchResult &result) {
  for (int i = 0; i < kBLeafCardinality; ++i) {
