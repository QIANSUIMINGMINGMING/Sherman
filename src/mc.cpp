...
namespace rdmacm
{
    namespace multicast
    {
        ...
        uint64_t check_package_loss(uint64_t psn_num)
        {
            ...
            // for (uint64_t j = 0; j < psn_num; j++)
            // {
            //     if (packages.find(std::make_pair(0, j)) == packages.end())
            //     {
            //         loss_packages++;
            //     } else {
            //         printf("psn number %lu \n", j);
            //     }
            // }
        ...
        multicastCM::multicastCM() : cnode_id(FLAGS_cnodeId), mcIp(FLAGS_mcIp), mcGroups(1), mbr(FLAGS_dramGB * FLAGS_rdmaMemoryFactor * 1024 * 1024 * 1024)
        {
            connect_memcached();
        ...
        int multicastCM::create_message(struct multicast_node *node)
        {
            ...
            node->recv_messages = (uint8_t *)mbr.allocate(kRecvMcPageSize * kMcMaxRecvPostList);
            // post initial recvs
            init_recvs(node);
            return 0;
        }
        ...
        void multicastCM::handle_recv(struct multicast_node *node, int id)
        {
            ...
            uint8_t *message = node->recv_messages + id * kRecvMcPageSize + 40; // ud padding
        ...
        int multicastCM::init_recvs(struct multicast_node *node)
        {
            ...
            for (int i = 0; i < kMcMaxRecvPostList; i++)
            {
                ...
                node->recv_wr[i].next = i == kMcMaxRecvPostList - 1 ? nullptr : &node->recv_wr[i + 1];
        ...
        void *multicastCM::mc_maintainer(uint16_t id, multicastCM *me)
        {
            ...
            while (true)
            {
                int num_comps = ibv_poll_cq(node->recv_cq, kpostlist, wc_buffer + recv_handle_pos);
                assert(num_comps >= 0);
                if (num_comps == 0)
                    continue;
                ...
                if (empty_recv_num >= kpostlist)
                {
                    ...
                    assert(ret == 0);
                    empty_start_pos = RING_ADD(empty_start_pos, empty_recv_num, kMcMaxPostList);
                    empty_recv_num = 0;
                }
            }
        }
