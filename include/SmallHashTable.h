...
class HashTable {
private:
    constexpr static int TABLE_SIZE = closest_prime(int(kLeafCardinality / 0.7)); // Chosen prime number
    std::vector<KeyValuePair> table;

    // Hash function
    int hash(uint64_t key) const {
        const double A = 0.6180339887;
        return static_cast<int>(TABLE_SIZE * (key * A - static_cast<uint64_t>(key * A)));
    }
...
public:
    ...
    void clear() {
        memset(table.data(), 0, sizeof(KeyValuePair) * TABLE_SIZE);
    }
